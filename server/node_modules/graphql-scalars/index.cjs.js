...
const error = require('graphql/error');
...
const DateTime$1 = new definition.GraphQLScalarType({
    name: 'DateTime',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING && ast.kind !== language.Kind.INT) {
            throw new error.GraphQLError(`Can only parse strings & integers to dates but got a: ${ast.kind}`);
        }
        const result = new Date(ast.kind === language.Kind.INT ? Number(ast.value) : ast.value);
        // eslint-disable-next-line no-restricted-globals
        if (Number.isNaN(result.getTime())) {
            throw new error.GraphQLError(`Value is not a valid Date: ${ast.value}`);
        }
        return result;
    },
});
...
const NonPositiveInt$1 = new definition.GraphQLScalarType({
    name: 'NonPositiveInt',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.INT) {
            throw new error.GraphQLError(`Can only validate integers as non-positive integers but got a: ${ast.kind}`);
        }
        return processValue(ast.value, VALIDATIONS.NonPositiveInt);
    },
});

const PositiveInt$1 = new definition.GraphQLScalarType({
    name: 'PositiveInt',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.INT) {
            throw new error.GraphQLError(`Can only validate integers as positive integers but got a: ${ast.kind}`);
        }
...
function NonNegativeIntFactory (name = 'NonNegativeInt') {
    return new definition.GraphQLScalarType({
        ...
        parseLiteral(ast) {
            if (ast.kind !== language.Kind.INT) {
                throw new error.GraphQLError(`Can only validate integers as non-negative integers but got a: ${ast.kind}`);
            }
...
const NegativeInt$1 = new definition.GraphQLScalarType({
    name: 'NegativeInt',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.INT) {
            throw new error.GraphQLError(`Can only validate integers as negative integers but got a: ${ast.kind}`);
        }
        return processValue(ast.value, VALIDATIONS.NegativeInt);
    },
});

const NonPositiveFloat$1 = new definition.GraphQLScalarType({
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.FLOAT && ast.kind !== language.Kind.INT) {
            throw new error.GraphQLError(`Can only validate floating point numbers as non-positive floating point numbers but got a: ${ast.kind}`);
        }
        return processValue(ast.value, VALIDATIONS.NonPositiveFloat);
    },
});

const PositiveFloat$1 = new definition.GraphQLScalarType({
    name: 'PositiveFloat',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.FLOAT && ast.kind !== language.Kind.INT) {
            throw new error.GraphQLError(`Can only validate floating point numbers as positive floating point numbers but got a: ${ast.kind}`);
        }
...
function NonNegativeFloatFactory (name = 'NonNegativeFloat') {
    return new definition.GraphQLScalarType({
        ...
        parseLiteral(ast) {
            if (ast.kind !== language.Kind.FLOAT && ast.kind !== language.Kind.INT) {
                throw new error.GraphQLError(`Can only validate floating point numbers as non-negative floating point numbers but got a: ${ast.kind}`);
            }
...
const NegativeFloat$1 = new definition.GraphQLScalarType({
    name: 'NegativeFloat',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.FLOAT && ast.kind !== language.Kind.INT) {
            throw new error.GraphQLError(`Can only validate floating point numbers as negative floating point numbers but got a: ${ast.kind}`);
        }
...
/* eslint-enable */
const EmailAddress$1 = new definition.GraphQLScalarType({
    name: 'EmailAddress',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as email addresses but got a: ${ast.kind}`);
        }
...
const URL$2 = new definition.GraphQLScalarType({
    name: 'URL',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as URLs but got a: ${ast.kind}`);
        }
...
const PhoneNumber$1 = new definition.GraphQLScalarType({
    name: 'PhoneNumber',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as phone numbers but got a: ${ast.kind}`);
        }
...
const PostalCode$1 = new definition.GraphQLScalarType({
    name: 'PostalCode',
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as postal codes but got a: ${ast.kind}`);
        }
...
function BigIntFactory (name = 'BigInt') {
    return new definition.GraphQLScalarType({
        ...
        parseLiteral(ast) {
            if (ast.kind === kinds.Kind.INT || ast.kind === kinds.Kind.FLOAT || ast.kind === kinds.Kind.STRING) {
...
const GUID$1 = new definition.GraphQLScalarType({
    name: `GUID`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as GUIDs but got a: ${ast.kind}`);
        }
...
const Hexadecimal$1 = new definition.GraphQLScalarType({
    name: `Hexadecimal`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as a hexadecimal but got a: ${ast.kind}`);
        }
...
const HexColorCode$1 = new definition.GraphQLScalarType({
    name: `HexColorCode`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as hex color codes but got a: ${ast.kind}`);
        }
...
const HSLA$1 = new definition.GraphQLScalarType({
    name: `HSLA`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as HSLA colors but got a: ${ast.kind}`);
        }
...
const IPv4$1 = new definition.GraphQLScalarType({
    name: `IPv4`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as IPv4 addresses but got a: ${ast.kind}`);
        }
...
const IPv6$1 = new definition.GraphQLScalarType({
    name: `IPv6`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as IPv6 addresses but got a: ${ast.kind}`);
        }
...
const ISBN$1 = new definition.GraphQLScalarType({
    name: `ISBN`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as ISBN numbers but got a: ${ast.kind}`);
        }
...
const MAC$1 = new definition.GraphQLScalarType({
    name: `MAC`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as MAC addresses but got a: ${ast.kind}`);
        }
...
const Port$1 = new definition.GraphQLScalarType({
    name: `Port`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.INT) {
            throw new error.GraphQLError(`Can only validate integers as TCP ports but got a: ${ast.kind}`);
        }
...
const RGB$1 = new definition.GraphQLScalarType({
    name: `RGB`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as RGB colors but got a: ${ast.kind}`);
        }
...
const RGBA$1 = new definition.GraphQLScalarType({
    name: `RGBA`,
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as RGBA colors but got a: ${ast.kind}`);
        }
 ...
 *    currency strings.
 */
const USCurrency$1 = new definition.GraphQLScalarType({
    name: 'USCurrency',
...
const JSON$2 = new definition.GraphQLScalarType({
    name: 'JSON',
...
const JSONObject$1 = new definition.GraphQLScalarType({
    name: 'JSONObject',
...
const IBAN$1 = new graphql.GraphQLScalarType({
    ...
    parseLiteral(ast) {
        if (ast.kind !== language.Kind.STRING) {
            throw new error.GraphQLError(`Can only validate strings as IBANs but got a: ${ast.kind}`);
        }
...
class RegularExpression extends definition.GraphQLScalarType {
    constructor(name, regex, options = {}) {
        ...
        super({
            ...
            parseLiteral(ast) {
                if (ast.kind !== language.Kind.STRING) {
                    throw new error.GraphQLError(`Can only validate strings as regular expressions but got a: ${ast.kind}`);
                }
